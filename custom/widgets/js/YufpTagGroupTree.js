...
(function (vue, $, name) {
  ...
  vue.component(name, {
    template: '<el-popover v-model="visiable" :width="width" :visible-arrow="false"\
           ...
           <yufp-ext-tree ref="tagGroupTree" :show-checkbox="needCheckbox" :height="height" :expand-level="expandLevel"\
           ...
           <el-input  ref="tagTreeInput" v-model="selectedVal" :readonly="readonly" :placeholder="placeholder" :disabled="disabled"\
    ...
    methods: {
      updateWidth: function () {
        if (this.$refs.tagTreeInput != null && this.$refs.tagTreeInput != undefined) {
          this.width = this.$refs.tagTreeInput.$el.scrollWidth;
        }
      },
      onIconClickFn: function () {
        ...
        if (!this.needCheckbox) {
          this.$refs.tagGroupTree.setCheckedKeys(this.tempNodekeys);
        }
        this.visiable = true;
      },
      ...
      confirmFn: function () {
        var me = this;
        var da = this.$refs.tagGroupTree.getCheckedKeys();
        if (da == null || da.length == 0) {
          me.$message('请至少选择一条数据!', '提示');
          return;
        }
        var tt = this.$refs.tagGroupTree.getCheckedNodes();
      ...
      refreshData: function () {
        this.$refs.tagGroupTree.remoteData();
      },
      // 对外提供选择器显示值
