...
(function (vue, $, name) {
    ...
    vue.component(name, {
        template: '<div>\
             ...
             @focus="onIconClickFn" :on-icon-click="onIconClickFn" readonly name="客户经理" v-model="selectedVal"></el-input>\
             <el-dialog-x title="客户经理选择器" :visible.sync= "dialogVisible" :height="height" :width = "width">\
             <el-row :gutter="20">\
        ...
        methods: {
            ...
            getDefaultData: function () {
                ...
                return {
                    ...
                    user: {
                        ...
                        maxHeight: maxHeight,
                        fieldData: [
                            { placeholder: '客户经理编号', field: 'userId', type: 'input' },
                            { placeholder: '客户经理名称', field: 'userName', type: 'input' }
                        ],
                        buttons: [{
                            label: '搜索',
                            op: 'submit',
                        ...
                        load: false, // 不默认查询
                        dataUrl: backend.appBaseService + '/api/pmafscheme/queryManagerList',
                        dataParams: {

                        },
                        tableColumns: [
                            { label: '客户经理编号', prop: 'userId', width: '', resizable: true },
                            { label: '客户经理名称', prop: 'userName', width: '', resizable: true }
                        ]
                    }
                };
            },
            ...
            queryFn: function (params) {
                ...
                console.log('params----->', params)
                var temp = params;
                temp.userId = !temp.userId ? this.user.dataParams.userId : temp.userId;
                temp.userName = !temp.userName ? this.user.dataParams.userName : temp.userName;
            ...
            confirmFn: function () {
                ...
                me.user.dataParams.userId = '';
                me.user.dataParams.userName = '';
...
}(Vue, yufp.$, 'yufp-manager-selector'));