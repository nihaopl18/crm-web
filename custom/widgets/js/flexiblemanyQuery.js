/**
 * yufp-single-many-query
 * 单一组合灵活查询
 * Created by linhongjie on 2022/01/17.
 */
(function (vue, $, name) {
    // 注册用户组件
    vue.component(name, {
    ...
    <div class="cust-form">\
        ...
        <div v-show="stepIndex === 2">\
            <div style="height: 600px">\
                <yu-xtable selection-type="checkbox" ref="yutable" reserve-selection row-key="custId" :data-url="custUrl" :default-load="false" :base-params="baseParam" style="width: 100%;margin-top: 16px" max-height="550" @selection-change="handleCustSelect">\
            ...
            nextStep: function (index) {
                if (index === 2) {
                    ...
                    for (let i = 0; i < this.stepTwoData.length; i++) {
                        if (i == 0) {
                            this.inputvalue = this.inputvalue + this.stepTwoData[i].custName + '-' + this.stepTwoData[i].custId;
                        }
                        if (i > 0) {
                            this.inputvalue = this.inputvalue + ';' + this.stepTwoData[i].custName + '-' + this.stepTwoData[i].custId;
                        }
...
}(Vue, yufp.$, 'yufp-single-many-query'));