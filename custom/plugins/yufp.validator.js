...
(function (yufp, window) {
  var validator = {
    ...
    'floatMax': function (rule, value, callback) {
      var reg = /^\d+(\.\d+)?$/;
      if (value > 100) {
        callback(new Error('数字类型错误'));
      } else if (value && !reg.test(value)) {
    ...
    /**
         ...
         * callback为回调函数,验证成功直接回调，验证失败回调函数返回一个带错误信息的Error实例
         * */
    'moneyFormat': function (rule, value, callback) {
    ...
    /**
     * 日期选择器结束日期不能小于当前日期，不能小于起始日期
     * @param {*} rule 为form表单当前验证的filed对应的验证rule规则
     * @param {*} value value为当前输入框返回值
     * @param {*} callback callback为回调函数,验证成功直接回调，验证失败回调函数返回一个带错误信息的Error实例
     */
    'endDate': function (rule, value, callback) {
      debugger;
      var date = new Date();
