...
(function (yufp, window, factory) {
  ...
  Service.prototype.request = function (options) {
    ...
    if (yufp.sessionStorage.get('selectRole')) {
      _options.headers.selectRole = yufp.sessionStorage.get('selectRole');
    }
  ...
  Service.prototype.getToken = function () {
    // alert((new Date().getTime()-nowDateTime)<10*60*1000);
    if ((new Date().getTime() - nowDateTime) > 30 * 60 * 1000) {
      // alert('退出');
