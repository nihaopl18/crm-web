            ...
            yufp.custom.vue({
                ...
                data: function() {
                    ...
                    return {
                        ...
                        // 左侧视图树
                        menuTreeUrl: backend.adminService + '/api/ocrmfsysviewmanager/getsysviewTree',
                        // 左侧视图树、右侧视图树请求参数
                        requestParams: { 'sysId': yufp.session.logicSys.id },
                        funcUrl: backend.adminService + '/api/ocrmfsysviewmanager/listquery',
                        ...
                        rules: {
                            // 菜单名称 校验
                            nameRules: [{ required: true, message: '必填项', trigger: 'blur' },
                            ...
                            // 上级节点 校验
                            upNodeRules: [{ required: true, message: '必填项', trigger: 'blur' }],
                            // 说明 校验
                            descriptionRules: [{ max: 23, message: '输入值过长', trigger: 'blur' }]
                        },
                        menuFields: [{
                            columnCount: 1,
                            fields: [{
                                ...
                                },
                                {
                                    ...
                                    focus: function(event) {
                                        _this.expandCollapseName = [];
                                        _this.expandCollapseName.push('funcList');
                                    },
                                    icon: 'circle-close',
                                    click: function() {
                                        _this.$refs.menuForm.formModel.funcId = '';
                                        _this.$refs.menuForm.formModel.funcName = '';
                                    }
                                },
                                {
                                    field: 'menuOrder',
                                ...
                                },
                                {
                                    ...
                                    focus: function(event) {
                                        _this.expandCollapseName = [];
                                        _this.expandCollapseName.push('upMenu');
                                    },
                        ...
                        formButtons: [{
                                ...
                                click: function(model, valid) {
                                    if (valid) {
                                        ...
                                        if (_this.addFlag || model.menuId == undefined) { // 新增
                                            model.sysId = yufp.session.logicSys.id;
                                            model.upMenuId = _this.currClickNode;
                                            yufp.service.request({
                                                ...
                                                callback: function(code, message, response) {
                                                    if (code == '0' && response.code == 0) {
                                                        ...
                                                        _this.$refs.menuForm.formModel.funcId = '';
                                                        delete _this.$refs.menuForm.formModel.menuId;
                                                    }
                                                }
                                            });
                                        } else { // 修改
                ...
                methods: {
                    ...
                    nodeClickFn: function(nodeData, node, self) {
                        ...
                        yufp.service.request({
                            method: 'GET',
                            data: param,
                            url: backend.adminService + '/api/ocrmfsysviewmanager/viewinfoquery',
                            callback: function(code, message, response) {
                                if (code == '0' && response.code == 0) {
                                    yufp.clone(response.data, _this.formdata);
                                    if (nodeData.pid == '0') {
                                        _this.formdata.parentName = _this.$refs.menuTree.data[0].label;
                                    }
                                } else {
                                    _this.$message('数据查询失败');
                                }
                            }
                        });
                    },
                    ...
                    /**
                     * 设置子节点维护—视图项-清空图标点击事件
                     */
                    iconClickFn: function() {
                        this.formdata.viewItemName = '';
                        this.formdata.viewItemId = '';
                    },
                    // 右侧菜单树，点击选择上层菜单
                    upMenuClickFn: function(nodeData, node, self) {
                        ...
                        var data = yufp.clone(this.formdata, {});
                        data.parentId = nodeData.id;
                        data.parentName = nodeData.label;
                        this.formdata = yufp.clone(data, this.formdata);
                    },
                    /**
                     * 点击新增按钮后的响应事件
                     */
                    createFn: function() {
                        ...
                        if (_this.currClickNode == '') {
                            _this.$message({ message: '请先选择视图节点', type: 'warning' });
                            return;
                        }
                        var temp = {
                            name: '',
                            ...
                            viewItemName: '',
                            orders: '',
                            parentId: _this.currClickNode,
                            parentName: _this.currClickName,
                            notes: '',
                            lastChgDt: '',
                            lastChgOrg: ''
                        };
                        yufp.clone(temp, _this.formdata);
                        // _this.formdata.viewItemId = '';
                        delete _this.formdata.id;
                    },
                    // 业务功能列表选择
                    funcSelect: function(row) {
                        this.formdata = yufp.clone({}, this.formdata);
                        this.formdata.viewItemId = row.id;
                        this.formdata.viewItemName = row.viewItemName;
                        // this.$refs.menuForm.formModel = yufp.extend({}, formModel);
                    },
                    // 删除菜单
                    deleteFn: function() {
                        if (this.currClickNode == '') {
                            this.$message({ message: '请先选择视图菜单节点', type: 'warning' });
                            return;
                        }
                        // 选择的视图菜单id
                        var viiewId = this.currClickNode;
                        var _this = this;
                        _this.$confirm('删除该菜单项的同时将删除其子菜单,确定删除?', '提示', {
                            ...
                            callback: function(action) {
                                if (action === 'confirm') {
                                    yufp.service.request({
                                        method: 'POST',
                                        url: backend.adminService + '/api/ocrmfsysviewmanager/deleteview',
                                        data: viiewId,
                                        callback: function(code, message, response) {
                                            if (code == '0' && response.code == 0) {
                                                ...
                                                _this.$refs.menuForm.resetFields();
                                                yufp.util.butLogInfo(hashCode, '视图树管理', '视图树删除');
                                            }
                                        }
                                    });
                                }
                    ...
                    /**
                     * 点击触发对应折叠面板
                     * @param {String} collId 折叠面板Id
                     */
                    activeCollFn: function(collId) {
                        this.expandCollapseName = [];
                        this.expandCollapseName.push(collId);
                    },
                    /**
                     * 保存
                     */
                    saveFn: function() {
                        ...
                        // 所属视图id，为右侧视图列表中的viewId
                        model.lastChgUsr = yufp.session.userCode;
                        // 逻辑系统id
                        // model.sysId = yufp.session.logicSys.id;
                        model.sysId = '1cab27def8fb4c0f9486dcf844b783c0';
                        model.lastChgOrg = yufp.session.org.code;
                        model.lastChgDt = new Date();
                        if (model.id == undefined) { // 新增
                            // 新增时，上级节点id为当前点击的节点id
                            model.parentId = _this.currClickNode;
                            yufp.service.request({
                                method: 'POST',
                                url: backend.adminService + '/api/ocrmfsysviewmanager/createview',
                                data: model,
                                callback: function(code, message, response) {
                                    if (code == '0' && response.code == 0) {
                                        _this.dialogVisible = false;
                                        ...
                                        _this.$refs.menuForm.resetFields();
                                        // _this.formdata.viewItemId = '';
                                        // delete _this.$refs.formdata.menuId;
                                        yufp.util.butLogInfo(hashCode, '视图树管理', '视图树新增');
                                    }
                                }
                            });
                        } else { // 修改
                            yufp.service.request({
                                method: 'POST',
                                url: backend.adminService + '/api/ocrmfsysviewmanager/editview',
                                data: model,
                                callback: function(code, message, response) {
                                    if (code == '0' && response.code == 0) {
                                        _this.dialogVisible = false;
                                        ...
                                        // _this.formdata.viewItemId = '';
                                        // delete _this.$refs.formdata.menuId;
                                        yufp.util.butLogInfo(hashCode, '视图树管理', '视图树修改');
                                    }
                                }
                            });
                        }
