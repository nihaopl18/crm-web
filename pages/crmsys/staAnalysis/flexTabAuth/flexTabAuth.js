...
*/
  exports.ready = function (hashCode, data, cite) {
    ...
    yufp.custom.vue({
      ...
      data: function () {
        return {
          ...
          editableTabsValue: '',
          pageable: true,
          notAutDataUrl: backend.adminService + '/api/ocrmacifqreportauth/getlist',
          autDataUrl: backend.adminService + '/api/ocrmacifqreportauth/getauthlist',
     ...
     */
        deleteFn: function () {
          ...
          for (var i = 0; i < deleteTem.length; i++) {
            ids.push(deleteTem[i].reportId);
          }
          ids = ids.join(',');
          yufp.service.request({
            method: 'POST',
            url: backend.adminService + '/api/ocrmacifqreportauth/deletereport/' + ids,
            data: comitData,
     ...
     * 选择角色
     */
        getTableFn: function (row, event, column, index) {
          var _this = this;
          _this.roleCode = { roleCode: row.roleId };
          _this.roleTem = row;
          // _this.autTabledata.splice(0);
          // _this.notAutTable.splice(0);
        },
        /**
     *获取已授权列表信息
     */
        ...
        addLeftFn: function () {
          ...
          for (var j = 0; j < _this.autTabledata.length; j++) {
            for (i = 0; i < selections.length; i++) {
              if (_this.autTabledata[j].reportId == selections[i].reportId) {
     ...
     */
        addRightFn: function () {
          ...
          for (var j = 0; j < _this.notAutTabledata.length; j++) {
            for (i = 0; i < selections.length; i++) {
              if (_this.notAutTabledata[j].reportId == selections[i].reportId) {
     ...
     */
        ...
        saveFn: function () {
          ...
          if (_this.roleTem.length < 1) {
            _this.$message({message: '请先选择一个角色！', type: 'waiting'});
            return;
          }
          ...
          yufp.service.request({
            method: 'POST',
            url: backend.adminService + '/api/ocrmacifqreportauth/savereport',
