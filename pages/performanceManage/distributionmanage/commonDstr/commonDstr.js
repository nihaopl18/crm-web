    ...
    yufp.custom.vue({
      ...
      watch: {
        dialogVisible: function (newVal) {
          ...
          if (newVal) {
            ...
            yufp.service.request({
              method: 'GET',
              url: backend.appBaseService + '/api/commondistribution/queryPeriodTable',
              ...
              callback: function (code, message, response) {
                ...
                for (var i = 0; i < _this.formDataTwo.length; i++) {
                  ...
                  Vue.set(_this.formDataTwo[i], 'list', []);
                  ...
                  yufp.service.request({
                    async: false,
                    method: 'GET',
                    url: backend.appBaseService + '/api/commondistribution/queryDistributeTable',
                    data: {condition: JSON.stringify(modelQuery)},
                    callback: function (code, message, response) {
                      _this.formDataTwo[i].list = response.data;
                    }
                  });
                }
              }
        ...
        dialogVisibleDetail: function (newVal) {
          ...
          if (newVal) {
            ...
            var urlPeriod = backend.appBaseService + '/api/commondistribution/queryPeriodTable';
            var urlDetail = backend.appBaseService + '/api/commondistribution/queryDistributeTable';
            if (obj.dstrSts == '03') {
              urlPeriod = backend.appBaseService + '/api/commondistribution/queryPeriodHisTableVet';
              urlDetail = backend.appBaseService + '/api/commondistribution/queryDistributeHisTable';
            }
            yufp.service.request({
              method: 'GET',
              url: urlPeriod,
      ...
      methods: {
        ...
        tempSave: function () {
          ...
          _this.formDataTwo[index].list = _this.contactData;
          _this.$message('暂存成功');
        },
        /**
         * 保存
        ...
        saveFn: function () {
          ...
          for (var i = 0; i < _this.formDataTwo.length; i++) {
            var dataModel = _this.formDataTwo[i].list;
          ...
          _this.$confirm('是否确定要保存?', '提示', {
            ...
            callback: function (action) {
              if (action === 'confirm') {
                ...
                yufp.service.request({
                  method: 'POST',
                  url: backend.appBaseService + '/api/commondistribution/saveData',
                  data: {
                    primaryValue: JSON.stringify(model),
                    funCode: data.funCode,
                    periodData: JSON.stringify(_this.formDataTwo),
                    quickPanel: 'true',
                    dataSrc: '01',
                    orgId: orgId
                  },
                  callback: function (code, message, response) {
                    if (response.flag) {
                      _this.$message({ message: '业绩分配失败，业绩正被操作中！', type: 'warning' });
                      return;
                    }
                    if (response.userEx) {
                      _this.$message({ message: '业绩分配失败，未找到下一节点办理人!', type: 'warning' });
                      return;
                    }
        ...
        addFn: function () {
          ...
          var obj = _this.$refs.yutable1.selections[0];
          if (obj.dstrSts != null && obj.dstrSts != undefined) {
            if (obj.dstrSts == '03') {
              _this.$message({ message: '选择的数据状态为待审批中', type: 'warning' });
              return;
            }
          }
          // 新加etl状态校验
        ...
        handleChaifen: function (index, row) {
          ...
          var obj = {
            ...
            operOrgName: yufp.session.details.grantOrgName,
        ...
        handleFenpei: function (index, row) {
          ...
          _this.$nextTick(function () {
            ...
            if (row.list.length > 0) {
              ...
              // 获取分配明细
              var url = backend.appBaseService + '/api/commondistribution/queryDistributeTable';
              if (_this.$refs.yutable1.selections[0].dstrSts == '03') {
        ...
        mgrSelectFn: function (data) {
          var _this = this;
          var obj = this.$refs.refTableThree.selections[0];
