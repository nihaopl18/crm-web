        ...
        yufp.custom.vue({
            ...
            data: function () {
                ...
                return {
                    ...
                    etlDate: '',
                    //dataUrl: backend.appBaseService + '/api/PmaFComDepAcctInfoResource/queryPmaFComDepAcctInfoList',
            ...
            watch: {
                dialogVisible: function (newVal) {
                    ...
                    if (newVal) {
                        ...
                        model.acctInfoNo = obj.id;
                        yufp.service.request({
                            method: 'GET',
                            url: backend.appBaseService + '/api/PmaFComDepPeriodResource/queryPmaFComDepPeriodList',
                            ...
                            callback: function (code, message, response) {
                                ...
                                for (var i = 0; i < _this.formDataTwo.length; i++) {
                                    /*if(_this.formDataTwo[i].applySts =='2'){
                                        _this.flagPeriod = false;
                                        _this.flagPeriodDelete = false;
                                        //_this.formDisabled = false;
                                        //this.refTableTwo.switch('effectDate', 'disabled', true);
                                    }*/
                                    ...
                                    Vue.set(_this.formDataTwo[i], 'pmaFcomDepDistributeInfo', []);
                                    ...
                                    yufp.service.request({
                                        async: false,
                                        method: 'GET',
                                        url: backend.appBaseService + '/api/PmaFcomDepDistributeResource/queryPmaFcomDepDistributeList',
                                        data: {condition: JSON.stringify(modelQuery)},
                                        callback: function (code, message, response) {
                                            _this.formDataTwo[i].pmaFcomDepDistributeInfo = response.data;
                                            //_this.contactData = _this.formDataTwo[i].pmaFcomDepDistributeInfo;
                ...
                dialogVisibleDetail: function (newVal) {
                    ...
                    if (newVal) {
                        ...
                        model.acctInfoNo = obj.id;
                        var urlPeriod = backend.appBaseService + '/api/PmaFComDepPeriodResource/queryPmaFComDepPeriodList';
                        var urlDetail = backend.appBaseService + '/api/PmaFcomDepDistributeResource/queryPmaFcomDepDistributeList';
            ...
            methods: {
                ...
                tempSave: function () {
                    ...
                    for (var i = 0; i < _this.contactData.length; i++) {
                        if (_this.contactData[i].managerId == '') {
                            this.$message({message: '请选择客户经理!', type: 'warning'});
                            return;
                        }
                    ...
                    _this.formDataTwo[index].pmaFcomDepDistributeInfo = _this.contactData;
                    _this.$message('暂存成功');
                },
                /**
                 * 保存
                ...
                saveFn: function () {
                    ...
                    for (var i = 0; i < _this.formDataTwo.length; i++) {
                        var dataModel = _this.formDataTwo[i].pmaFcomDepDistributeInfo;

                        for (var j = 0; j < dataModel.length; j++) {
                            if (dataModel[j].managerId == '') {
                                this.$message({message: '第' + (i + 1) + '个区间请选择客户经理!', type: 'warning'});
                                modeChild = true;
                                break;
                            }
                        ...
                        callback: function (action) {
                            if (action === 'confirm') {
                                ...
                                yufp.service.request({
                                    ...
                                    data: {
                                        workType: '1',
                                        acctInfoId: DepAcctInfoPV,
                                        periodList: _this.formDataTwo
                                    },
                                    callback: function (code, message, response) {
                                        if (code == 0) {
                                            var dataPids = response.data;
                                            _this.$refs.yutable1.remoteData();
                                            _this.$message('操作成功');
                                            _this.dialogVisible = false;

                                            _this.commitFn2(DepAcctInfoPV,applySts,dataPids);
                ...
                commitFn2: function(approveId,applySts,dataPids){
                    var _this = this;
                    /*var str = "";
                    for(var i=0;i<dataPids.length;i++){
                        str += dataPids[i]+",";
                    }*/
                    ...
                    commitData.applType = 'CK_YJFP'; // 审批流程
                ...
                addFn: function () {
                    ...
                    if(obj.applySts == undefined ||obj.applySts == "3"){
                        _this.flagCsh = true;
                    }else {
                        _this.flagCsh = false;
                    }

                    //_this.maxAmt = obj.balance;
                ...
                handleFenpei: function (index, row) {
                    var _this = this;
                    if(row.applySts == undefined ||row.applySts == "3"){
                        _this.flagCsh = true;
                    }else {
                        _this.flagCsh = false;
                    }
                    ...
                    _this.$nextTick(function () {
                        ...
                        // 获取分配明细
                        var url = backend.appBaseService + '/api/PmaFcomDepDistributeResource/queryPmaFcomDepDistributeList';
                        yufp.service.request({
                            method: 'GET',
                            url: url,
                            data: {condition: JSON.stringify(model)},
                ...
                handleThreeDelete: function (index, row) {
                    if (this.flagOne) {
                        ...
                        if (flag) {
                            ...
                            if(key.length == 0){
                                var indexMax = this.contactData.length -1;
                                this.contactData[indexMax].endAmt = row.endAmt;
                            }
                        }
                ...
                changeMoney: function (index) {
                    ...
                    if (parseInt(endAmt) > parseInt(oldendAmt)) {
                        this.contactData[index].endAmt = oldendAmt;
                        this.$message({message: '输入金额大于最大余额', type: 'warning'});
                        return;
                    }
                    var virManagerId = 'vm' + yufp.session.org.code;
                    var obj = {};
                ...
                commitFn: function () {
                    ...
                    var str = "";
                    var dataPids = ['805e2f99db7845d9917bc499ea34e618'];
                    for(var i=0;i<dataPids.length;i++){
                        str += dataPids[i]+",";
                    }

                    commitData.paramMap = {
                        periodId: JSON.stringify(str)
                    };
                    _this.$refs.yufpWfInit.wfInit(commitData);
                },
