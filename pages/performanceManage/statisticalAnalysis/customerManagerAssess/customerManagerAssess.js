    ...
    yufp.custom.vue({
      ...
      data: function () {
        return {
          ...
          dataUrl: backend.appBaseService + '/api/pmafschemeindexscoremgr/mgrAssessSelf',
          scoreDataUrl: backend.appBaseService + '/api/pmafschemeindexscoremgr/mgrAssessIndexInfo',
      ...
      methods: {
        ...
        scoreSearchFn: function () {
          ...
          model.managerId = selectData.managerId;
        ...
        searchFn: function () {
          ...
          _this.$refs['refQuery'].validate(function (valid) {
            if (valid) {
              ...
              // model.managerId = yufp.session.userId;
              ...
              //     // 15->客户经理  R002->客户经理(理财) R003->客户经理(个贷) R004->团队长
              //     if (roles[i].code != '15' && roles[i].code != 'R002' && roles[i].code != 'R003') {
              //         //有一个角色不是客户经理
        ...
        downloadFn: function () {
          ...
          if (_this.$refs.refTable.selections.length > 0) { // 导出选中数据
            ...
            for (var i = 0; i < selectData.length; i++) {
              // var obj = {};
              // obj.managerId = selectData[i].managerId;
              // obj.managerName = selectData[i].managerName;
            ...
            yufp.util.download('/api/pmafschemeindexscoremgr/mgrAssessExportExcelAll?' + reqData);
          } else { // 导出全量
            let reqData = '';
            let validate = true;
            _this.$refs['refQuery'].validate(function (valid) {
              if (valid) {
                ...
                reqData = reqData + 'schemeId=' + encodeURIComponent(formdata.schemeId) + '&managerId=' + encodeURIComponent(formdata.managerId) + '&rankMethod=' + encodeURIComponent(formdata.rankMethod);
              } else {
                validate = false;
                return;
              }
        ...
        performanceFn: function () {
          ...
          formdata.teamId = selectData.teamId;
          formdata.schemeCycle = selectData.schemeCycle;
          formdata.managerId = selectData.managerId;
          formdata.objType = '01';
          yufp.service.request({
            method: 'GET',
            ...
            callback: function (code, message, response) {
              if (response.code == 0) {
                ...
                yufp.service.request({
                  ...
                  callback: function (code, message, response) {
                    if (response.code == 0) {
                      _this.listData = [].concat(response.data);
