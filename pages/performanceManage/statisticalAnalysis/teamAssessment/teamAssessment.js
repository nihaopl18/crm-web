    ...
    yufp.custom.vue({
      ...
      data: function () {
        return {
          ...
          dataUrl: backend.appBaseService + '/api/pmafschemeindexscoreteamresource/teamAssessSelf',
          scoreDataUrl: backend.appBaseService + '/api/pmafschemeindexscoreteamresource/teamAssessIndexInfo',
      ...
      methods: {
        ...
        scoreSearchFn: function () {
          ...
          model.teamId = selectData.teamId;
        ...
        downloadFn: function () {
          ...
          if (_this.$refs.refTable.selections.length > 0) { // 导出选中数据
            ...
            for (var i = 0; i < selectData.length; i++) {
              // var obj = {};
              // obj.teamId = selectData[i].teamId;
              // obj.teamName = selectData[i].teamName;
            ...
            yufp.util.download('/api/pmafschemeindexscoreteamresource/teamAssessExportExcelAll?' + reqData);
          } else { // 导出全量
            let reqData = '';
            let validate = true;
            _this.$refs['refQuery'].validate(function (valid) {
              if (valid) {
                ...
                reqData = reqData + 'schemeId=' + encodeURIComponent(formdata.schemeId) + '&teamId=' + encodeURIComponent(formdata.teamId) + '&rankMethod=' + encodeURIComponent(formdata.rankMethod);
              } else {
                validate = false;
                return;
              }
