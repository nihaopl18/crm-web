/**
 * @created by chenlin on 2019-3-14 10:09:04
 * @updated by
 * @description 秒杀组件
 */
define(['pages/climp_qy/marketingMode/secKill/secKill.css',
  ...
  exports.ready = function (hashCode, data, cite) {
    yufp.lookup.reg('MUST_FIELDS,RELATE_PROD_TYPE');
    yufp.custom.vue({
      ...
      methods: {
        ...
        saveFn: function () {
          var _this = this;
          var validate = false;
          this.$refs.sekillform.validate(function (valid) {
            validate = valid;
          });
          if (!validate) {
            return;
          ...
          var preData = [];
          for (var i = 0; i < this.$refs.sekillform.fields.length; i++) {
            var preObj = {};
            preObj.formOperationFiled = this.$refs.sekillform.fields[i].name;// 'startTime';
            if (this.$refs.sekillform.fields[i].name == 'startTime' || this.$refs.sekillform.fields[i].name == 'endTime') {
              preObj.formOperationVal = yufp.util.dateFormat(this.$refs.sekillform.fields[i].fieldValue, '{y}-{m}-{d} {h}:{i}:{s}');
            } else if (this.$refs.sekillform.fields[i].name == 'mastItems' || this.$refs.sekillform.fields[i].name == 'markPic') {
              if (this.$refs.sekillform.fields[i].fieldValue != null && this.$refs.sekillform.fields[i].fieldValue != '') {
                preObj.formOperationVal = this.$refs.sekillform.fields[i].fieldValue.join(',');
              }
              preObj.formOperationVal = this.$refs.sekillform.fields[i].fieldValue;
            }
            preData.push(preObj);
          }
          yufp.service.request({
        ...
        preview: function () {
          var _this = this;
          _this.dialogImageUrl1 = 'pages/climp_qy/marketingMode/assemble/assemble.jpg';
          ...
          yufp.service.request({
            ...
            callback: function (code, message, response) {
              ...
              for (var i = 0; i < from.length; i++) {
                // for (var j = 0; j < _this.$refs.sekillform.fields.length; j++) {
