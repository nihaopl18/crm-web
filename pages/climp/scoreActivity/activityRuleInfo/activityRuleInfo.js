...
define([
  ...
  exports.ready = function (hashCode, data, cite) {
    var activityId = data.bizSeqNo;
    yufp.lookup.reg('USE_FLAG,WF_APP_STATUS,SCORE_TYPE,MAXSCORE_TYPE');
    yufp.custom.vue({
      el: cite.el,
      data: function () {
        return {
          tabName: 'activityInfo',
          disabled: true,
          transactionCode: '',
          isCollapseShow: false, // 折叠面板是否显示
          activityInfoFormdata: {}, // 活动详情表单数据
          props: {
            key: 'fieldEName',
            value: 'fieldCName'
          },
        ...
        yufp.service.request({
          ...
          callback: function (code, message, response) {
            _this.$nextTick(function () {
              yufp.clone(response.data[0], _this.activityInfoFormdata); // 加载活动详情信息
            });
            _this.transactionCode = (response.data[0] && response.data[0].transactionCode) || '';
            _this.querySelectFn();
      ...
      methods: {
        ...
        /**
         * 重置ruleFormPanel，主要涉及规则信息，计算方式，动作配置
         */
        ...
        infoFn: function () {
          ...
          _this.$nextTick(function () {
            ...
            yufp.clone(selectionsAry[0], _this.actionConfigFormdata); // 回显动作配置表单数据
            // 处理计算方式数据
            _this.setFormulaWayForm(selectionsAry[0] || '1');
            // 处理积分条件数据反显
