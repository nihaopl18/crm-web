      ...
      data: function () {
        ...
        return {
          ...
          tableColumns: [
            ...
            { label: '开始时间', prop: 'begintime', resizable: true},
          ...
          updateFields: [{
            ...
            fields: [
              ...
              { field: 'vicarioustype',
                label: '委托类型',
                type: 'select',
                options: vicarioustypeOptions,
                rules: [
                  {required: true, message: '必填项', trigger: 'blur'}],
                change: function (vicarioustype) {
                  var field = me.updateFields[0].fields;
                  if (vicarioustype == '0') { // 默认指定
                    field[7].hidden = true;
                    field[7].rules[0].required = false;
                  } else if (vicarioustype == '1') { // 特殊指定
                    field[7].hidden = false;
                    field[7].rules[0].required = true;
                  }
                }
              },
              { field: 'appid',
        ...
        openEditFn: function (row) { // 打开修改页面
          ...
          if (row.userid == yufp.session.user.loginCode) {
            ...
            this.$nextTick(function () {
              ...
              this.$refs.wfHumanstatesForm.formModel.begintime = this.getDate(row.begintime);
              this.$refs.wfHumanstatesForm.formModel.endtime = this.getDate(row.endtime);
            });
          } else {
        ...
        saveCreateFn: function () { // 新增保存
          ...
          myform.validate(function (valid) {
            if (valid) {
              ...
              if (comitData.vicarioustype == '0') {
                comitData.appid = '';
              }
              comitData.begintime = me.beginTime;
              comitData.endtime = me.endTime;
                    ...
                    yufp.service.request({
                      ...
                      callback: function (code, message, response) {
                        ...
                        if (response.data.flag == 'success') {
                          me.$refs.wfHumanstatesForm.resetFields();
