    ...
    yufp.custom.vue({
      ...
      data: function () {
        ...
        return {
          ...
          tpId: '',
          checkbox: true,
          DutyParams: {sessionInstuCde: yufp.session.instu.code},
          tableColumns: [
            ...
            ]},
          {
            ...
            fields: [
              { field: 'tpdsc',
                ...
                type: 'textarea',
                rows: 1,
                rules: [{max: 100, message: '最大长度不超过100个字符', trigger: 'blur' }]
              }
            ]
          }]
        };
        ...
        openDutyFn: function () { // 打开关联岗位页面
          ...
          this.$nextTick(function () {
            ...
            yufp.service.request({
              method: 'GET',
              url: backend.echainService + '/api/jointaskpool/queryAllDutysQry',
              data: me.DutyParams,
                      ...
                      if (users) {
                        for (var i = 0; i < users.length; i++) {
                          ...
                          for (var k = 0; k < allDuty.length; k++) {
                            if (users[i].relatedCode == allDuty[k].dutyCde) {
                              dutyName = allDuty[k].dutyName;
                            }
                          }
                          var option = {
                            dutyCde: users[i].relatedCode,
                            dutyName: dutyName
                          };
                          me.checkDutyList.push(option);
                        }
                      }
                    }
                  });
                    ...
                    yufp.service.request({
                      ...
                      callback: function (code, message, response) {
                        if (response.data == true) {
                          ...
                          yufp.util.butLogInfo(hashCode, '项目池管理', '新增');
                    ...
                    yufp.service.request({
                      ...
                      callback: function (code, message, response) {
                        if (response.data == true) {
                          ...
                          yufp.util.butLogInfo(hashCode, '项目池管理', '修改');
                          me.$refs.WfTaskPoolList.remoteData();
                        } else {
              ...
              yufp.service.request({
                ...
                callback: function (code, message, response) {
                  if (response.data == true) {
                    ...
                    yufp.util.butLogInfo(hashCode, '项目池管理', '删除');
                    me.$refs.WfTaskPoolList.remoteData();
                  } else {
          ...
          yufp.service.request({
            ...
            callback: function (code, message, response) {
              ...
              if (response.data.flag == 'success') {
                me.$message({message: response.data.message, type: response.data.flag});
                me.dialogVisibleDuty = false;
                yufp.util.butLogInfo(hashCode, '项目池管理', '关联岗位设置');
                me.checkDutyList = [];
              }
            }
          });
