            ...
            methods: {
                ...
                exportFn: function () {
                    ...
                    for (let i = 0; i < em.selectRows.length; i++) {
                        if (em.selectRows[i].ecifCustNo) {
                            exportIds.push(em.selectRows[i].ecifCustNo)
                        }
                    }
                    if ((exportIds && exportIds.length != 0 && exportIds.length < 50001) || em.pageData.total < 50001) {
                        em.exportObj.exportIds = exportIds && exportIds.join();
                        em.exportObj.selectRole = yufp.sessionStorage.get('selectRole');
                        var url = '/api/OcrmFGiGroup/export?' + 'condition=' + encodeURI(JSON.stringify(em.exportObj));
                        yufp.util.download(url);
                        yufp.util.butLogInfo(hashCode, '管户报表', '导出');
                    } else {
                        em.$message({
                            showClose: true,
