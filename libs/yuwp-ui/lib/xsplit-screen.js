...
// EXTERNAL MODULE: ./packages/carousel/src/main.vue + 4 modules
var main = __webpack_require__(74);

// EXTERNAL MODULE: ./packages/carousel/src/item.vue + 4 modules
var src_item = __webpack_require__(65);

// EXTERNAL MODULE: ./packages/xsplit-screen/src/split-screen-item.vue + 2 modules
var split_screen_item = __webpack_require__(59);
...
// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib??vue-loader-options!./packages/xsplit-screen/src/split-screen.vue?vue&type=script&lang=js&
...
/* harmony default export */ var split_screenvue_type_script_lang_js_ = ({
  name: 'YuXsplitScreen',
  ...
  component: {
    ElCarousel: main["a" /* default */],
    ElCarouselItem: src_item["a" /* default */],
    YuXsplitScreenItem: split_screen_item["a" /* default */]
  },
  props: {
    dialogHeight: {
      type: String,
    ...
    // 分屏相关设置属性，是否轮播，轮播时间登，具体参考Carousel 组件。
    initialIndex: {
      type: Number,
      default: 0
    },
  ...
  watch: {
    screenData: function screenData(data) {
      var cloneTemplate = {};
      cloneTemplate = external_deepmerge_default()(cloneTemplate, data, { clone: true });
      this.template = this.orderTemplateData(cloneTemplate);
    }
  },
  ...
  methods: {
    // 对模版数据进行排序
    orderTemplateData: function orderTemplateData(template) {
    ...
    getTemplateData: function getTemplateData() {
      ...
      yufp.service.request({
        url: _this.screenDataUrl,
        data: _this.baseParam,
        method: _this.requestType,
        async: false,
        callback: function callback(code, message, response) {
          var templateArr = Object(util_["getValueByPath"])(response, _this.jsonData) || [];
          var map = {};
          var dest = [];
          for (var i = 0; i < templateArr.length; i++) {
            var ai = templateArr[i];
            if (!map[ai.pageNo]) {
              dest.push({
                pageNo: ai.pageNo,
                data: [ai]
          ...
          if (!dest.length > 0) dest.push({ data: [], pageNo: 0 });
          _this.initGraphList = external_deepmerge_default()(cloneTemplate, dest, { clone: true });
          _this.template = _this.orderTemplateData(dest);
          _this.$emit('load-end', _this.template);
        }
      });
    },
    // 用于数组排序的方法
    ...
    handleDelete: function handleDelete(event, item, index, $index) {
      event.stopPropagation();
      this.template[$index].data.splice(index, 1);
      this.$emit('delete-end', item, index, $index);
    },
    handleMore: function handleMore(event, item, index, $index) {
      event.stopPropagation();
      this.$emit('handle-more', item, index, $index);
    },
    handleEnlarge: function handleEnlarge(event, item, index, $index) {
      event.stopPropagation();
      this.curItem = item;
      this.$nextTick(function () {
    ...
    handDropEnd: function handDropEnd(template) {
      this.$emit('drop-end', template);
    },
  ...
  mounted: function mounted() {
    for (var i = 0, l = this.template.length; i < l; i++) {
      var item = this.template[i];
      for (var j = 0, lh = item.data.length; j < lh; j++) {
        var subitem = item.data[j];
        if (subitem.type === 'router') {
          yufp.router.to(subitem.routerId, '', subitem.routerId);
        }
      }
    }
  },
  created: function created() {
    if (this.screenDataUrl) {
      ...
      this.template = this.orderTemplateData(this.screenData);
    }
  },
  ...
  render: function render(h) {
    ...
    return h('div', [h(
      main["a" /* default */],
      external_babel_helper_vue_jsx_merge_props_default()([{ props: props }, {
        on: {
          'change': _this.change
        }
      }]),
      [_this.$slots.custom ? h(src_item["a" /* default */], [_this.$slots.custom]) : '', _this._l(_this.template, function (items, $index) {
        return h(
          src_item["a" /* default */],
          { 'class': 'drag-container' },
          [_this._l(items.data, function (item, index) {
            return h(
              'div',
              { 'class': 'drag-item', style: _this.layout },
              [h(
                'div',
                { 'class': 'drag-box', style: { height: parseFloat(_this.height) / Math.ceil(items.data.length / _this2.column) - 40 + 'px' } },
                [h(
                  'div',
                  { 'class': 'title' },
                  [h('span', [item.graphName]), h('i', {
                    'class': 'el-icon-d-arrow-right',
                    on: {
                      'click': function click(event) {
                        return _this.handleMore(event, item, index, $index);
                      }
                    }
                  }), h('i', {
                    ...
                    on: {
                      'click': function click(event) {
                        return _this.handleDelete(event, item, index, $index);
                      }
                    }
                  }), h('i', {
                    ...
                    on: {
                      'click': function click(event) {
                        return _this.handleEnlarge(event, item, index, $index);
                      }
                    }
                  })]
                ), item.type === 'router' ? h(split_screen_item["a" /* default */], external_babel_helper_vue_jsx_merge_props_default()([{ props: item }, {
                  attrs: {
                    id: item.routerId
                  }
                }])) : h(split_screen_item["a" /* default */], { props: item })]
              )]
            );
          })]
        );
      ...
      external_babel_helper_vue_jsx_merge_props_default()([{
        attrs: {
          ...
          title: _this.curItem.graphName,
          visible: _this.dialogVisible
        }
      }, { on: { 'update:visible': function updateVisible(val) {
            return _this.dialogVisible = val;
          } } }]),
      [_this.curItem.type === 'router' ? h(split_screen_item["a" /* default */], external_babel_helper_vue_jsx_merge_props_default()([{ props: _this.curItem }, {
        attrs: {
          id: _this.curItem.routerId
        }
      }])) : h(split_screen_item["a" /* default */], { props: _this.curItem })]
    )]);
  }
});
// CONCATENATED MODULE: ./packages/xsplit-screen/src/split-screen.vue?vue&type=script&lang=js&
 /* harmony default export */ var src_split_screenvue_type_script_lang_js_ = (split_screenvue_type_script_lang_js_); 
// EXTERNAL MODULE: ./node_modules/vue-loader/lib/runtime/componentNormalizer.js
var componentNormalizer = __webpack_require__(1);

// CONCATENATED MODULE: ./packages/xsplit-screen/src/split-screen.vue
var split_screen_render, staticRenderFns
...
var component = Object(componentNormalizer["a" /* default */])(
  src_split_screenvue_type_script_lang_js_,
  split_screen_render,
  staticRenderFns,
  false,
  null,
...
/* hot reload */
if (false) { var api; }
component.options.__file = "packages/xsplit-screen/src/split-screen.vue"
/* harmony default export */ var split_screen = (component.exports);
// CONCATENATED MODULE: ./packages/xsplit-screen/index.js


split_screen.install = function (Vue) {
  Vue.component(split_screen.name, split_screen);
};

/* harmony default export */ var xsplit_screen = __webpack_exports__["default"] = (split_screen);
  ...
  return _c(
    "div",
    {
      ...
      class: {
        ...
        "is-in-stage": !_vm.active && _vm.inStage,
        "is-hover": _vm.hover,
  ...
  methods: {
    ...
    handleItemClick: function handleItemClick() {
      ...
      if (parent && parent.type === 'card') {
        ...
        var oldIndex = parent.activeIndex;
        var cond1 = index === parent.items.length - 1 && oldIndex === 0;
        var cond2 = index === 0 && oldIndex === parent.items.length - 1;
...
var render = function() {
  ...
  return _c(
    ...
    [
      ...
      _vm.indicatorPosition !== "none" && _vm.indicator
        ? _c(
            "ul",
...
/* harmony default export */ var mainvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    resetItemPosition: function resetItemPosition(oldIndex) {
      ...
      // 防止修改addResizeListener后传element对象进来产生影响
      var oldVal = (typeof oldIndex === 'undefined' ? 'undefined' : _typeof(oldIndex)) === 'object' ? undefined : oldIndex;
      this.items.forEach(function (item, index) {
        item.translateItem(index, _this2.activeIndex, oldVal);
      });
    },
