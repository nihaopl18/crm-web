...
/* harmony default export */ var tab_barvue_type_script_lang_js_ = ({
  ...
  props: {
    tabs: Array,
    // 增加tab大小，供统一配置是否紧凑调整,该参数内部使用,需要配置样式一起处理
    size: {
      type: Number,
      default: 12
    }
        ...
        this.tabs.every(function (tab, index) {
          ...
          if (!tab.active) {
            ...
            if (sizeName === 'width' && _this.tabs.length > 1) {
              tabSize -= index === 0 || index === _this.tabs.length - 1 ? _this.size : _this.size * 2;
            }
            return false;
          }
        });
...
/* harmony default export */ var tab_navvue_type_script_lang_js_ = ({
  ...
  render: function render(h) {
    ...
    var type = this.type,
        ...
        onTabDblClick = this.onTabDblClick,
...
/* harmony default export */ var tabsvue_type_script_lang_js_ = ({
  ...
  methods: {
    handleTabDblClick: function handleTabDblClick(tab, tabName, event) {
      if (tab.disabled) return;
      this.setCurrentName(tabName);
      this.$emit('tab-dblclick', tab, event);
    },
    handleTabContextMenu: function handleTabContextMenu(tab, tabName, event) {
      if (tab.disabled) return;
      this.$emit('tab-contextmenu', tab, event);
    },
    handleTabMouseLeave: function handleTabMouseLeave(tab, tabName, event) {
      if (tab.disabled) return;
      this.$emit('tab-mouseleave', tab, event);
    },
  ...
  render: function render(h) {
    ...
    var type = this.type,
        ...
        handleTabDblClick = this.handleTabDblClick,
        handleTabContextMenu = this.handleTabContextMenu,
        handleTabMouseLeave = this.handleTabMouseLeave,
    ...
    var navData = {
      props: {
        ...
        onTabDblClick: handleTabDblClick,
