...
/* harmony default export */ var collapse = __webpack_exports__["a"] = (component.exports);
            ...
            "div",
            {
              ...
              on: {
                "!click": function($event) {
                  $event.stopPropagation()
                  $event.preventDefault()
                  return _vm.handleHeaderClick($event)
                }
              }
            },
            [
              _vm.showTitleClass
                ? _c(
                    "span",
                    { staticClass: "el-collapse-item-icon-title-wrap" },
                    [
                      _c("i", {
                        staticClass:
  ...
  computed: {
    isActive: function isActive() {
      if (this.$parent.accordion) {
        return this.$parent.activeNames[0] === this.name;
      } else {
...
/* harmony default export */ var collapse_item = __webpack_exports__["a"] = (component.exports);
...
// EXTERNAL MODULE: ./packages/xform/src/form-item-x.vue + 7 modules
var form_item_x = __webpack_require__(77);

// EXTERNAL MODULE: ./packages/xform/src/form.vue + 3 modules
var src_form = __webpack_require__(66);

// EXTERNAL MODULE: ./packages/xform/src/form-group.vue + 2 modules
var form_group = __webpack_require__(67);

// EXTERNAL MODULE: ./packages/collapse/src/collapse.vue + 4 modules
var collapse = __webpack_require__(72);

// EXTERNAL MODULE: ./packages/collapse/src/collapse-item.vue + 4 modules
var collapse_item = __webpack_require__(73);
...
// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib??vue-loader-options!./packages/xdynamic-form/src/form.vue?vue&type=script&lang=js&
...
/* harmony default export */ var formvue_type_script_lang_js_ = ({
  ...
  component: {
    YuXform: src_form["a" /* default */],
    YuXformItem: form_item_x["a" /* default */],
    YuXformGroup: form_group["a" /* default */],
    ElCollapse: collapse["a" /* default */],
    ElCollapseItem: collapse_item["a" /* default */]
  },
  props: {
    sort: {
      type: Boolean,
  ...
  watch: {
    templateData: function templateData(data) {
      ...
      var template = this.orderTemplateData(cloneTemplate);
      this.template = this.transferTemplate(template);
    }
  },

  methods: {
    // 因数组类型数据用克隆赋值的方式监听不到，统一采用extend方法赋值
    setFormData: function setFormData(data) {
      yufp.extend(this.formdata, data);
    },
    ...
    // 对模版数据中的linkage，以及校验做转换
    transferTemplate: function transferTemplate(templateArr) {
      for (var i = 0, l = templateArr.length; i < l; i++) {
        var fields = templateArr[i].fields;
        for (var j = 0, lh = fields.length; j < lh; j++) {
          var field = fields[j];
          if (field.linkage && typeof field.linkage === 'string') {
            field.linkage = Object(util["b" /* str2obj */])(field.linkage);
          }
          if (field.rules instanceof Array) {
            for (var k = 0, _l = field.rules.length; k < _l; k++) {
              var rule = field.rules[k];
              if (rule.validator) {
                rule.validator = Object(util["b" /* str2obj */])(rule.validator);
              }
            }
          }
          if (field.rules instanceof Object) {
            if (field.rules.validator) {
              field.rules.validator = Object(util["b" /* str2obj */])(field.rules.validator);
            }
          }
        }
      }
    ...
    orderTemplateData: function orderTemplateData(template) {
      this.activeArr = [];
      template = this.sort ? template.sort(this.compare('order')) : template;
      for (var i = 0, l = template.length; i < l; i++) {
        template[i].fields = this.sort ? template[i].fields.sort(this.compare('order')) : template[i].fields;
        if (template[i].collapse) {
          this.activeArr.push(template[i].title);
        }
      }
      return template;
    },
    ...
    getTemplateData: function getTemplateData() {
      var _this = this;
      yufp.service.request({
        url: _this.templateDataUrl,
        data: _this.baseParam,
        method: _this.requestType,
        async: false,
        callback: function callback(code, message, response) {
          var templateArr = Object(util_["getValueByPath"])(response, _this.jsonData) || [];
          _this.template = _this.orderTemplateData(_this.transferTemplate(templateArr));
        }
      });
    },
    // 用于数组排序的方法
    ...
    renderFormItem: function renderFormItem(h, field, formitem, formindex) {
      var _this2 = this;
      ...
      delete cloneFormItem.value;
      if (cloneFormItem.linkage) {
        return h('yu-xform-item', external_babel_helper_vue_jsx_merge_props_default()([{ props: cloneFormItem }, {
          attrs: { value: value, name: cloneFormItem.field, colspan: cloneFormItem.colspan, ctype: cloneFormItem.ctype },
          on: {
            'change': function change() {
              return cloneFormItem.linkage(_this2, _this2.formdata, field, _this2.templateData);
            }
          }
        }]));
      } else {
  ...
  created: function created() {
    if (this.templateDataUrl) {
      this.getTemplateData();
    } else {
      this.template = this.orderTemplateData(this.transferTemplate(this.templateData));
    }
  },
  ...
  render: function render(h) {
    ...
    if (this.mode === 'collapse') {
      return h(
        src_form["a" /* default */],
        external_babel_helper_vue_jsx_merge_props_default()([{ ref: 'dynamicForm', attrs: { 'label-width': _this.labelWidth, dynamic: true, mode: 'collapse', value: _this.formdata
          },
          on: {
            'input': function input($$v) {
              _this.formdata = $$v;
        ...
        [h(
          ...
          [_this._l(_this.template, function (item, $index) {
            return h(
              collapse_item["a" /* default */],
              {
                attrs: { title: item.title, name: item.title }
              },
              [h(
                form_group["a" /* default */],
                {
                  attrs: { column: item.column }
                },
                [_this._l(item.fields, function (formitem, formindex) {
                  return _this.renderFormItem.call(_this3._renderProxy, h, item.fields, formitem, formindex);
                })]
              )]
            );
          })]
      ...
      return h(
        src_form["a" /* default */],
        external_babel_helper_vue_jsx_merge_props_default()([{ ref: 'dynamicForm', attrs: { 'label-width': _this.labelWidth, dynamic: true, mode: 'normal', value: _this.formdata
          },
          on: {
            'input': function input($$v) {
              _this.formdata = $$v;
...
// CONCATENATED MODULE: ./packages/xdynamic-form/src/form.vue?vue&type=script&lang=js&
 /* harmony default export */ var src_formvue_type_script_lang_js_ = (formvue_type_script_lang_js_); 
// EXTERNAL MODULE: ./node_modules/vue-loader/lib/runtime/componentNormalizer.js
var componentNormalizer = __webpack_require__(1);

// CONCATENATED MODULE: ./packages/xdynamic-form/src/form.vue
...
/* hot reload */
if (false) { var api; }
component.options.__file = "packages/xdynamic-form/src/form.vue"
/* harmony default export */ var xdynamic_form_src_form = (component.exports);
// CONCATENATED MODULE: ./packages/xdynamic-form/index.js


/* istanbul ignore next */
xdynamic_form_src_form.install = function (Vue) {
  Vue.component(xdynamic_form_src_form.name, xdynamic_form_src_form);
};

/* harmony default export */ var xdynamic_form = __webpack_exports__["default"] = (xdynamic_form_src_form);
